...
class Sphere
{
	public:
		...
		double reflectivity = 0, refractivity=0;
		double mu_refract = 1;
		...
		Vector get_refraction(Vector, Vector);
};
...
Vector Sphere::get_refraction(Vector incoming, Vector point)
{
	// Vector normal = get_normal(point);
	return incoming;	
}
...
class Polygon
{
	public:
		...
		double reflectivity = 0.5, refractivity=0;
		double mu_refract = 1;
...
Vector Polygon::get_refraction(Vector incoming, Vector point)
{
	return incoming;
}
...
class Screen
{
	public:			
		...
		Vec3b recursive_intersection(Vector, Vector, double);
};
...
Vec3b Screen::recursive_intersection(Vector ray_pt, Vector ray_dir, double factor)
{
	if(factor < 0.001) 
	{
		// cout<<"here1"<<endl;
	...
	else
	{
		...
		if(index.first != -1)
		{
			...
			Vec3b reflect = recursive_intersection(intersect, reflected, reflectivity*factor);
			for(int l = 0; l< 3; l++)
			{
...
Mat Screen::gen_image()
{
	...
	for(int i=0; i<pixel_dim.first; i++)
	{
		for(int j=0; j<pixel_dim.second; j++)
		{
			...
			img(j,i) = recursive_intersection(ray_pt, ray_udir, 1);
