...
class Plane
{
	public:
		...
		double D;
		Colour colour;
		Plane(double, Vector, Colour);
		double get_intersection(Vector, Vector);	// Get intersection point of ray with plane
		Vector get_normal(Vector);
};

Plane::Plane(double D_set = 0, Vector normal_set = Vector(0,0,1,1), Colour colour_set = Colour(0,255,0))
{
	D = D_set;
...
double Plane::get_intersection(Vector ray_pt, Vector ray_dir)
{
	Vector ray_udir = ray_dir.unit();
	double numerator = -((normal^ray_pt) + D);
	double denominator = (normal^ray_udir);
	double t = numerator/denominator;
	if(t<0) printf("Plane before screen");
	Vector p = ray_pt + (ray_udir*t);
	return t;
}

// Get normal of plane.
Vector Plane::get_normal(Vector point)
{	
	return normal;
}
...
class Triangle
{
	public:
		...
		int affined ;
		Mat affine,inverse,tpose;
		Triangle(Vector, Vector, Vector, Colour);
		double get_intersection(Vector, Vector);	// Get intersection point of ray with plane
		Vector get_normal(Vector);
};

Triangle::Triangle(Vector p1_set = Vector(120,0,0,1), Vector p2_set = Vector(100,20,0,1),
					 Vector p3_set = Vector(80,0,0,1), Colour colour_set = Colour(0,0,255))
{
	points[0] = p1_set;
	points[1] = p2_set;
	points[2] = p3_set;
...
double Triangle::get_intersection(Vector ray_pt, Vector ray_dir)
{
	// Construct plane from points // Doubt in direction of normal
	Vector normal = ((points[1]-points[0])&(points[2] - points[0])).unit();
	double D = (points[0] - Vector(0,0,0,0))^normal;
	...
	double del1 = ((p - points[0])&(p - points[1])).mag();
	double del2 = ((p - points[1])&(p - points[2])).mag();
	double del3 = ((p - points[2])&(p - points[0])).mag();
	double del = ((points[2] - points[0])&(points[1] - points[0])).mag();




	if(abs(del1 + del2 + del3 - del) > 0.01) return  100000000;
...
Polygon::Polygon(vector<Vector> vertices_set = points, Colour colour_set = Colour(10,10,200))
{
	colour = colour_set;
...
Vec3b Screen::calculate_intensity(Vector point, Vector camera, Vector norm, Colour colour, vector<Vector> lights)
{
	...
	for(int i=0;i<lights.size();i++)
	{
		...
		{
			double t = triangles[j].get_intersection(point,incident);
	...
	for(int i=0; i<pixel_dim.first; i++)
	{
		for(int j=0; j<pixel_dim.second; j++)
		{
			...
			for(int k=0; k<spheres.size(); k++)
			{
				...
				double t_tmp = t_min;
				if(t_s.first > 0) t_min = min(t_min, t_s.first);
				else if(t_s.second > 0) t_min = min(t_min, t_s.second);
				if(abs(t_min - t_tmp) > 0.001) index = make_pair(0,k);
			}

			for(int k=0; k<triangles.size(); k++)
			{
				double t = triangles[k].get_intersection(ray_pt, ray_udir);
				double t_tmp = t_min;
				if(t > 0) t_min = min(t_min, t);
				if(abs(t_min - t_tmp) > 0.001) index = make_pair(1,k);
			}
			...
			if(abs(t_min - 10000000.0) > 0.01 ) count++;

			if(index.first != -1)
			{
				Vector intersect = ray_pt + (ray_udir*t_min);
				
				// Bad indentation but who cares !
				...
				switch(index.first)
				{
					case 0:
						...
						break;				
					case 1:
						norm = triangles[index.second].get_normal(intersect);
						colour= triangles[index.second].colour;
						break;
					case 2:
				...
				img(j,i) = calculate_intensity(intersect,camera,norm,colour, lights);
			}
...
/*	
	...
	affine_global.at<float>(3,3)=1.0;
	affine_global.at<float>(0,0)=0.5;
	affine_global.at<float>(0,2)=0.5;
	affine_global.at<float>(1,0)=0.5;
	affine_global.at<float>(1,1)=0.5;
	affine_global.at<float>(2,1)=1;
	affine_global.at<float>(2,2)=0.5;

*/

	affine_matrix_feature();

	S.affined=1;
	S.affine = affine_global.clone(); 
	S.inverse = inverse_global.clone();
	S.tpose = tpose_global.clone();
...
// Define here the affine matrix for polygon object.
void input_polygon(Polygon& P)
{
	...
	P.affined = 1;
	P.affine  = affine_global.clone();
	P.inverse = inverse_global.clone();
	P.tpose = tpose_global.clone();
